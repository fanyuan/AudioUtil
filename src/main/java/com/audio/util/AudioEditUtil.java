...
public class AudioEditUtil {
   ...
   * @param progress2 附加音频音频强度
   */
  public static void mixAudioWithSameBaseSrcAudio(Audio srcAudio, Audio coverAudio, Audio outAudio, float startTime, float progress1, float progress2) {
    ...
    try {
      ...
      final int srcStartPos = getPositionFromWave(startTime, sampleRate, channels, bitNum);
      final int srcEndPos = (int) (srcFis.length() - WAVE_HEAD_SIZE);
      final int coverStartPos = 0;
      final int coverEndPos = (int) coverFis.length() - WAVE_HEAD_SIZE;
      ...
      //混合音频指定时间段的数据
      int copyCoverSize = coverEndPos - coverStartPos;
      int copySrcSize = srcEndPos - srcStartPos;
      int times = 0;
      if(copySrcSize >  copyCoverSize){
        times = copySrcSize/copyCoverSize;
      }
      int size = (int) (srcFis.getChannel().size() - WAVE_HEAD_SIZE);
      Log.d("ddebug","mix audio times = " + times + "---size = " + size + "---srcStartPos=" + srcStartPos);
      copyData(srcFis, newFos, srcStartPos);//copyData(srcFis, newFos, copyCoverSize/2);//
      mixData(srcFis, coverFis, newFos, copyCoverSize, progress1, progress2);
